...
const Section4 = forwardRef((props, ref) => {
  ...
  const handlePersonChange = (id, field, value) => {
    const newPeople = people.map((person) => {
      if (person.id === id) {
        return { ...person, [field]: value, [`${field}State`]: "" };
      }
      return person;
    });
    setPeople(newPeople);
  ...
  const validateFields = () => {
    ...
    const validatedPeople = people.map((person) => {
      ...
      if (!verifyLength(person.name)) {
        newPerson.nameState = "has-danger";
        isAllValid = false;
      } else {
        newPerson.nameState = "has-success";
      }
      if (!verifyLength(person.role)) {
        newPerson.roleState = "has-danger";
        isAllValid = false;
      } else {
        newPerson.roleState = "has-success";
      }
      if (!person.image) {
        newPerson.imageState = "has-danger";
        isAllValid = false;
      } else {
        newPerson.imageState = "has-success";
      }
      return newPerson;
    });
      ...
      <CardBody>
        ...
        {people.map((person, index) => (
          <div key={person.id}>
            ...
            <Row>
              <Col md="8">
                <FormGroup className={`has-label ${person.nameState}`}>
                  ...
                  />
                  {person.nameState === "has-danger" && (
                    <Label className="error">This field is required.</Label>
                  )}
                </FormGroup>
                <FormGroup className={`has-label ${person.roleState}`}>
                  <Label>Person role</Label>
                  <Input
                    type="text"
                    value={person.role}
                    onChange={(e) =>
                      handlePersonChange(person.id, "role", e.target.value)
                    }
                  />
                  {person.roleState === "has-danger" && (
                    <Label className="error">This field is required.</Label>
                  )}
                </FormGroup>
              </Col>
              <Col md="4">
                ...
                <FormGroup className={person.imageState}>
                  <ImageUpload
                    onFileChange={(file) =>
                      handlePersonChange(person.id, "image", file)
                    }
                  />
                  {person.imageState === "has-danger" && (
